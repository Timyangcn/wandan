...
async function predict_classification()
 {
  ...
  classification = ["qualified", "failed", "feature"]
  ...
  pred.data()
    ...
    document.getElementById("output_text").innerHTML = "<p>判斷輪胎是否通過: " + CLASSIFICATION_DETECTED + "(" + (max_val*100).toFixed(2) + "% probability)</p>"
  }) 
